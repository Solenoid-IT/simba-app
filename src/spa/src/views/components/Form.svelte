    ...
    $:
        ...
        {// Value found
            ...
            api.reset = function ()
            {
                ...
                // (Iterating each entry)
                element.querySelectorAll('.form-widget').forEach
                (
                    function (el)
                    {
                ...
                // (Iterating each entry)
                element.querySelectorAll('.table-component').forEach
                (
                    function (el)
                    {
                        // (Resetting the element)
            ...
            api.validate = function ()
            {
                ...
                const result =
                {
                    ...
                    'fetch':   function ()
                    {
                        ...
                        // Returning the value
                        return FlexObject.expand( values );
                    }
                }
                ;
                ...
                (
                    function (inputElement)
                    {
                        ...
                        if ( inputElement.classList.contains( 'monaco-editor' ) ) return;
                        ...
                        switch ( inputElement.getAttribute('data-type') )
                        {
                            case 'string':
                                // (Getting the value)
                                result.entries[ name ].value = result.entries[ name ].value.toString();
                            break;

                            case 'int':
                                // (Getting the value)
                                result.entries[ name ].value = result.entries[ name ].value === '' ? null : parseInt( result.entries[ name ].value );
                            break;

                            case 'float':
                                // (Getting the value)
                                result.entries[ name ].value = result.entries[ name ].value === '' ? null : parseFloat( result.entries[ name ].value );
                            break;
                        }
                    }
                )
                ...
                // (Iterating each entry)
                element.querySelectorAll( '.form-input.monaco-editor' ).forEach
                (
                    function (el)
                    {
                        // (Getting the values)
                        const name  = el.getAttribute('data-input');
                        const value = el.api.getValue();
                        ...
                        result.entries[ name ] =
                        {
                            ...
                            'element': el,
                            'valid':   el.getAttribute('data-required') === null ? true : value !== ''
                        }
                        ;
                ...
                // (Iterating each entry)
                element.querySelectorAll( '.form-widget[data-input]' ).forEach
                (
                    function (el)
                    {
                        // (Getting the values)
                        ...
                        result.entries[ name ] =
                        {
                            ...
                            'element': el.querySelector( '.input' ) ?? el,
                            'valid':   el.getAttribute( 'data-required' ) === null ? true : value !== ''
                        }
                        ;
                ...
                // (Iterating each entry)
                element.querySelectorAll('.select-box[data-input]').forEach
                (
                    function (el)
                    {
                        // (Getting the values)
                        ...
                        result.entries[ name ] =
                        {
                            ...
                            'element': el,
                            'valid':   el.getAttribute('data-required') === null ? true : value !== null
                        }
                        ;
                        ...
                        {// (Entry is valid)
                            // (Removing the class)
                            result.entries[ name ].element.querySelector('.form-input').classList.remove( 'input-invalid' );
                        }
                        else
                        {// (Entry is not valid)
                            // (Adding the class)
                            result.entries[ name ].element.querySelector('.form-input').classList.add( 'input-invalid' );
                        }
                    }
                )
                ;
            ...
            api.setValues = function (values)
            {
                ...
                {// Processing each entry
                    ...
                    {// Value not found
                        ...
                        {// Value not found
                            // (Getting the value)
                            el = element.querySelector(`.form-widget[data-input="${ k }"]`);
